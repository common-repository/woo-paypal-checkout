 ...
 * @subpackage Woo_Paypal_Checkout/public
 * @author     paltechwpdev <paltechwpdev@gmail.com>
 */
class Woo_Paypal_Checkout_Public {
    ...
    public function enqueue_scripts() {
        ...
        if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
            return;
        }
        ...
        if (is_product() && $this->show_on_product_page == 'yes') {
            ...
            $this->commit = "true";
            $this->page = "checkout";
            wp_enqueue_script('paltechwpdevwpc_checkout_page', PALTECHWPDEVWPC_ASSET_URL . 'public/js/woo-paypal-checkout-page.js', array('jquery'), $this->version, false);
        }
        ...
        wp_register_script('paltechwpdevwpc_checkout_sdk', $paypal_sdk, array('jquery'), null, false);
        wp_register_script('paltechwpdevwpc_checkout_sdk-frontend', PALTECHWPDEVWPC_ASSET_URL . 'public/js/woo-paypal-checkout-public' . $suffix . '.js', array('jquery'), $this->version, false);
        wp_localize_script('paltechwpdevwpc_checkout_sdk-frontend', 'paltechwpdevwpc_param', array(
            'page' => $this->page,
            'layout' => $this->layout,
            'color' => $this->color,
            'shape' => $this->shape,
            ...
            'add_to_cart_ajaxurl' => WC_AJAX::get_endpoint('paltechwpdevwpc_ajax_generate_cart'),
            'get_checkout_details' => add_query_arg(array('pp_action' => 'get_checkout_details', 'utm_nooverride' => '1'), WC()->api_request_url('WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout')),
            'set_checkout' => add_query_arg(array('pp_action' => 'set_checkout', 'utm_nooverride' => '1'), WC()->api_request_url('WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout')),
            'display_order_page' => add_query_arg(array('pp_action' => 'display_order_page', 'utm_nooverride' => '1'), WC()->api_request_url('WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout')),
            'cancel_url' => add_query_arg(array('pp_action' => 'cancel_url', 'utm_nooverride' => '1'), WC()->api_request_url('WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout')),
                )
        );
    }

    public function paltechwpdevwpc_display_paypal_button_on_product_page() {
        global $product;
        if (!is_product()) {
            return;
        }
        ...
        wp_enqueue_script('paltechwpdevwpc_checkout_sdk');
        wp_enqueue_script('paltechwpdevwpc_checkout_sdk-frontend');
        ?>
        <div class="paltechwpdevwpc_paypal_checkout_div">
            <div id="paltechwpdevwpc_paypal_button_product"></div>
        </div>
        <?php
    }

    public function paltechwpdevwpc_display_paypal_button_on_cart_page() {
        if (WC()->cart->is_empty()) {
            return;
        }
        if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
        ...
        ?>
        <div class="paltechwpdevwpc_paypal_checkout_div">
            <div id="paltechwpdevwpc_paypal_button_cart"></div>
            <div class="paltechwpdevwpc-checkout-buttons__separator">
    ...
    public function paltechwpdevwpc_display_paypal_button_on_checkout_page() {
        if (WC()->cart->is_empty()) {
            return;
        }
        if (paltechwpdevwpc_has_active_session() == true) {
            return false;
        }
        if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
            return;
        ...
        ?>
        <div class="paltechwpdevwpc_paypal_checkout_div">
            <div id="paltechwpdevwpc_paypal_button_checkout"></div>
        </div>
        <?php
    }

    public function paltechwpdevwpc_is_credit_supported() {
        $base = wc_get_base_location();
        return 'US' === $base['country'] && 'USD' === get_woocommerce_currency();
    }

    public function paltechwpdevwpc_add_header_meta() {
        if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
            return;
        }
    ...
    public function paltechwpdevwpc_enable_paypal_checkout() {
        $gateways = WC()->payment_gateways->get_available_payment_gateways();
        if (!isset($gateways['paltechwpdevwpc_paypal_checkout'])) {
            return false;
        }
        if (isset($gateways['paltechwpdevwpc_paypal_checkout']) && is_object($gateways['paltechwpdevwpc_paypal_checkout'])) {
            $this->gateway = $gateways['paltechwpdevwpc_paypal_checkout'];
        }
    ...
    public function paltechwpdevwpc_ajax_generate_cart() {
        ...
        $url = esc_url_raw(add_query_arg('pp_action', 'set_express_checkout', add_query_arg('wc-api', 'WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout', home_url('/'))));
        try {
            ...
            if (is_object($product)) {
                ...
                if ($product->is_type('variable')) {
                    ...
                    $bool = $this->paltechwpdevwpc_is_product_already_in_cart($product->get_id(), $qty, $variation_id, $attributes);
                    if ($bool == false) {
                        WC()->cart->add_to_cart($product->get_id(), $qty, $variation_id, $attributes);
                    }
                } elseif ($product->is_type('simple')) {
                    $bool = $this->paltechwpdevwpc_is_product_already_in_cart($product->get_id(), $qty);
                    if ($bool == false) {
                        WC()->cart->add_to_cart($product->get_id(), $qty);
                    }
                }
                WC()->cart->calculate_totals();
                $this->paltechwpdevwpc_init_sdk();
                $this->request->paltechwpdevwpc_create_order_request();
            }
        } catch (Exception $ex) {
    ...
    public function paltechwpdevwpc_is_product_already_in_cart($product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array()) {
    ...
    public function paltechwpdevwpc_init_sdk() {
        try {
            include_once PALTECHWPDEVWPC_PLUGIN_DIR . '/includes/gateway/class-wc-gateway-paltechwpdevwpc-paypal-checkout-request.php';
            if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
                return;
            }
            $this->request = new WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request($this->gateway);
        } catch (Exception $ex) {
            
        }
    }

    public function paltechwpdevwpc_add_body_classes($classes) {
        if (is_checkout()) {
            if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
                return;
            }
            ...
            $classes[] = 'paltechwpdevwpc_paypal_checkout';
            if (paltechwpdevwpc_has_active_session()) {
                $classes[] = 'paltechwpdevwpc_paypal_checkout_hidden';
            }
        }
        return $classes;
    }

    public function paltechwpdevwpc_endpoint_page_titles($title) {
        if (!is_admin() && is_main_query() && in_the_loop() && is_page() && is_checkout() && paltechwpdevwpc_has_active_session()) {
    ...
    public function paltechwpdevwpc_maybe_disable_other_gateways($gateways) {
        if (paltechwpdevwpc_has_active_session()) {
            foreach ($gateways as $id => $gateway) {
                if ('paltechwpdevwpc_paypal_checkout' !== $id) {
                    unset($gateways[$id]);
                }
            }
            if (is_cart() || ( is_checkout() && !is_checkout_pay_page() )) {
                if (isset($gateways['paltechwpdevwpc_paypal_checkout']) && ( 0 >= WC()->cart->total )) {
                    unset($gateways['paltechwpdevwpc_paypal_checkout']);
                }
            }
        }
        if (is_cart() || ( is_checkout() && !is_checkout_pay_page() )) {
    ...
    public function paltechwpdevwpc_checkout_details_to_post() {
        if ($this->paltechwpdevwpc_enable_paypal_checkout() == false) {
            return;
        }
        if (!paltechwpdevwpc_has_active_session()) {
        ...
        $this->paltechwpdevwpc_init_sdk();
        try {
            $paltechwpdevwpc_order_details = WC()->session->get('paltechwpdevwpc_order_details');
            if (empty($paltechwpdevwpc_order_details)) {
                $paltechwpdevwpc_order_id = WC()->session->get('paltechwpdevwpc_order_id');
                $this->request->paltechwpdevwpc_get_checkout_details($paltechwpdevwpc_order_id);
                $paltechwpdevwpc_order_details = WC()->session->get('paltechwpdevwpc_order_details');
            }
        } catch (Exception $e) {
        ...
        $shipping_details = $this->paltechwpdevwpc_get_mapped_shipping_address($paltechwpdevwpc_order_details);
        $billing_details = $this->paltechwpdevwpc_get_mapped_billing_address($paltechwpdevwpc_order_details);
    ...
    public function paltechwpdevwpc_get_mapped_billing_address($checkout_details) {
        ...
        if (!empty($checkout_details['purchase_units'][0]['shipping']['address'])) {
            ...
            $state = $this->paltechwpdevwpc_get_state_code($country, $state_value);
        } else {
            $company = '';
            $address_1 = '';
            $address_2 = '';
    ...
    public function paltechwpdevwpc_get_mapped_shipping_address($checkout_details) {
    ...
    public function paltechwpdevwpc_maybe_add_shipping_information($packages) {
        if (!paltechwpdevwpc_has_active_session()) {
            return $packages;
        }
        $paltechwpdevwpc_order_details = WC()->session->get('paltechwpdevwpc_order_details');
        ...
        $destination = $this->paltechwpdevwpc_get_mapped_shipping_address($paltechwpdevwpc_order_details);
    ...
    public function paltechwpdevwpc_checkout_fields($checkout_fields) {
        if (paltechwpdevwpc_has_active_session()) {
            $this->paltechwpdevwpc_init_sdk();
            try {
                $paltechwpdevwpc_order_details = WC()->session->get('paltechwpdevwpc_order_details');
            ...
            if (true === WC()->cart->needs_shipping_address()) {
                $shipping_details = $this->paltechwpdevwpc_get_mapped_billing_address($paltechwpdevwpc_order_details);
    ...
    public function paltechwpdevwpc_formatted_billing_address() {
        if (!paltechwpdevwpc_has_active_session()) {
            return;
        }
        try {
            ...
            $billing_address = $this->paltechwpdevwpc_get_mapped_billing_address($paltechwpdevwpc_order_details);
        } catch (Exception $e) {
            wc_add_notice($e->getMessage(), 'error');
            return;
        }
    ...
    public function paltechwpdevwpc_formatted_shipping_address() {
        if (!paltechwpdevwpc_has_active_session()) {
            return;
        }
        try {
            ...
            $shipping_address = $this->paltechwpdevwpc_get_mapped_shipping_address($paltechwpdevwpc_order_details);
        } catch (Exception $e) {
            wc_add_notice($e->getMessage(), 'error');
            return;
        }
    ...
    public function paltechwpdevwpc_get_state_code($country, $state) {
        try {
    ...
    public function paltechwpdevwpc_woocommerce_checkout_get_value($value, $checkout_key) {
        $paltechwpdevwpc_order_details = WC()->session->get('paltechwpdevwpc_order_details');
        $shipping_address = $this->paltechwpdevwpc_get_mapped_shipping_address($paltechwpdevwpc_order_details);
        $billing_address = $this->paltechwpdevwpc_get_mapped_billing_address($paltechwpdevwpc_order_details);
