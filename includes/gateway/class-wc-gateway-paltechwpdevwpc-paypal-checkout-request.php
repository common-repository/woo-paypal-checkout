 ...
 * PayPal Gateway.
 *
 * @class       WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout
...
class WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request {

    public $paltechwpdevwpc_gateway;

    public function __construct($gateway) {
        $this->gateway = $gateway;
        $this->paltechwpdevwpc_sandbox = 'yes' === $this->gateway->get_option('sandbox', 'no');
        if ($this->paltechwpdevwpc_sandbox) {
            $this->is_sandbox = 'Yes';
            $this->paltechwpdevwpc_client_id = $this->gateway->get_option('rest_client_id_sandbox', '');
            $this->paltechwpdevwpc_secret_id = $this->gateway->get_option('rest_secret_id_sandbox', '');
            ...
            $this->basicAuth = base64_encode($this->paltechwpdevwpc_client_id . ":" . $this->paltechwpdevwpc_secret_id);
            $this->paltechwpdevwpc_access_token = get_transient('paltechwpdevwpc_sandbox_access_token', false);
            if ($this->paltechwpdevwpc_access_token == false) {
                $this->paltechwpdevwpc_get_access_token();
            }
        } else {
            $this->is_sandbox = 'No';
            $this->paltechwpdevwpc_client_id = $this->gateway->get_option('rest_client_id_live', '');
            $this->paltechwpdevwpc_secret_id = $this->gateway->get_option('rest_secret_id_live', '');
            ...
            $this->basicAuth = base64_encode($this->paltechwpdevwpc_client_id . ":" . $this->paltechwpdevwpc_secret_id);
            $this->paltechwpdevwpc_access_token = get_transient('paltechwpdevwpc_live_access_token', false);
            if ($this->paltechwpdevwpc_access_token == false) {
                $this->paltechwpdevwpc_get_access_token();
            }
        }
    ...
    public function paltechwpdevwpc_get_access_token() {
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('PayPal Checkout for WooCommerce Version: %s', 'woo-paypal-checkout'), WOO_PAYPAL_CHECKOUT_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('WooCommerce Version: %s', 'woo-paypal-checkout'), WC_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_oauth_api);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Get an access token => https://developer.paypal.com/docs/api/overview/#get-an-access-token');
        ...
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Get access token Request' . $this->paypal_oauth_api);
        if (is_wp_error($response)) {
            $error_message = $response->get_error_message();
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Error Message : ' . wc_print_r($error_message, true));
        } else {
            $api_response = json_decode(wp_remote_retrieve_body($response), true);
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response Code: ' . wp_remote_retrieve_response_code($response));
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response Message: ' . wp_remote_retrieve_response_message($response));
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response Body: ' . wc_print_r($api_response, true));
            if ($this->paltechwpdevwpc_sandbox) {
                set_transient('paltechwpdevwpc_sandbox_access_token', $api_response['access_token'], 29000);
            } else {
                set_transient('paltechwpdevwpc_live_access_token', $api_response['access_token'], 29000);
            }
            $this->paltechwpdevwpc_access_token = $api_response['access_token'];
        }
    }

    public function paltechwpdevwpc_create_order_request($woo_order_id = null) {
        if ($woo_order_id == null) {
            $cart = $this->paltechwpdevwpc_get_details_from_cart();
        } else {
            $cart = $this->paltechwpdevwpc_get_details_from_order($woo_order_id);
        }
        $reference_id = wc_generate_order_key();
        WC()->session->set('paltechwpdevwpc_reference_id', $reference_id);
        $body_request = array(
            'intent' => 'CAPTURE',
            'purchase_units' =>
            array(
        ...
        $body_request = paltechwpdevwpc_remove_empty_key($body_request);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('PayPal Checkout for WooCommerce Version: %s', 'woo-paypal-checkout'), WOO_PAYPAL_CHECKOUT_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('WooCommerce Version: %s', 'woo-paypal-checkout'), WC_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_order_api);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Create order => https://developer.paypal.com/docs/api/orders/v2/#orders_create');
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Request : ' . wc_print_r($body_request, true));
        ...
        $response = wp_remote_post($this->paypal_order_api, array(
            ...
            'httpversion' => '1.0',
            'blocking' => true,
            'headers' => array('Content-Type' => 'application/json', 'Authorization' => "Bearer " . $this->paltechwpdevwpc_access_token, "prefer" => "return=representation"),
            'body' => $body_request,
            'cookies' => array()
                )
        );
    ...
    public function paltechwpdevwpc_order_capture_request($woo_order_id) {
        $order = wc_get_order($woo_order_id);
        $this->paltechwpdevwpc_update_order($order);
        $paypal_order_id = WC()->session->get('paltechwpdevwpc_order_id');
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('PayPal Checkout for WooCommerce Version: %s', 'woo-paypal-checkout'), WOO_PAYPAL_CHECKOUT_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('WooCommerce Version: %s', 'woo-paypal-checkout'), WC_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id . '/capture');
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Capture payment for order  => https://developer.paypal.com/docs/api/orders/v2/#orders_capture');
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Request : ' . wc_print_r($this->paypal_order_api . $paypal_order_id . '/capture', true));
        ...
        if (is_wp_error($response)) {
            ...
            $return_response = array();
            $api_response = json_decode(wp_remote_retrieve_body($response), true);
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response : ' . wc_print_r($api_response, true));
            $return_response['orderID'] = $api_response['id'];
    ...
    public function paltechwpdevwpc_get_checkout_details($paypal_order_id) {
        if (WC()->cart->is_empty()) {
            return;
        }
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('PayPal Checkout for WooCommerce Version: %s', 'woo-paypal-checkout'), WOO_PAYPAL_CHECKOUT_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('WooCommerce Version: %s', 'woo-paypal-checkout'), WC_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Show order details  => https://developer.paypal.com/docs/api/orders/v2/#orders_get');
        ...
        if (is_wp_error($response)) {
            ...
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response Message: ' . wp_remote_retrieve_response_message($response));
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Response Body: ' . wc_print_r($api_response, true));
            WC()->session->set('paltechwpdevwpc_order_id', $paypal_order_id);
            WC()->session->set('paltechwpdevwpc_order_details', $api_response);
        }
    }

    public function paltechwpdevwpc_get_details_from_cart() {
        $old_wc = version_compare(WC_VERSION, '3.0', '<');
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        $rounded_total = $this->paltechwpdevwpc_get_rounded_total_in_cart();
        ...
        $details = array(
            ...
            'items' => $this->paltechwpdevwpc_get_paypal_line_items_from_cart(),
            'shipping_address' => $this->paltechwpdevwpc_get_address_from_customer(),
            'email' => $old_wc ? WC()->customer->billing_email : WC()->customer->get_billing_email(),
        );
        return $this->paltechwpdevwpc_get_details($details, $discounts, $rounded_total, WC()->cart->total);
    }

    public function paltechwpdevwpc_get_number_of_decimal_digits() {
        return $this->paltechwpdevwpc_is_currency_supports_zero_decimal() ? 0 : 2;
    }

    public function paltechwpdevwpc_is_currency_supports_zero_decimal() {
        return in_array(get_woocommerce_currency(), array('HUF', 'JPY', 'TWD'));
    }

    public function paltechwpdevwpc_get_rounded_total_in_cart() {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        $rounded_total = 0;
    ...
    public function paltechwpdevwpc_get_paypal_line_items_from_cart() {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        $items = array();
        foreach (WC()->cart->cart_contents as $cart_item_key => $values) {
            $amount = round($values['line_subtotal'] / $values['quantity'], $decimals);
    ...
    public function paltechwpdevwpc_get_address_from_customer() {
    ...
    public function paltechwpdevwpc_get_details($details, $discounts, $rounded_total, $total) {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        ...
        if (abs($lisum) > 0.000001 && 0.0 !== (float) $diff) {
            $details['items'][] = $this->paltechwpdevwpc_get_extra_offset_line_item($details['total_item_amount'] - $lisum);
        }
        return $details;
    }

    public function paltechwpdevwpc_get_details_from_order($order_id) {
        $order = wc_get_order($order_id);
        $decimals = $this->paltechwpdevwpc_is_currency_supports_zero_decimal() ? 0 : 2;
        $rounded_total = $this->paltechwpdevwpc_get_rounded_total_in_order($order);
        ...
        $details = array(
            ...
            'items' => $this->paltechwpdevwpc_get_paypal_line_items_from_order($order),
        );
        $details = $this->paltechwpdevwpc_get_details($details, $order->get_total_discount(), $rounded_total, $order->get_total());
        $old_wc = version_compare(WC_VERSION, '3.0', '<');
        if (( $old_wc && ( $order->shipping_address_1 || $order->shipping_address_2 ) ) || (!$old_wc && $order->has_shipping_address() )) {
            $shipping_first_name = $old_wc ? $order->shipping_first_name : $order->get_shipping_first_name();
            $shipping_last_name = $old_wc ? $order->shipping_last_name : $order->get_shipping_last_name();
    ...
    public function paltechwpdevwpc_get_rounded_total_in_order($order) {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
    ...
    public function paltechwpdevwpc_get_paypal_line_items_from_order($order) {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        $order = wc_get_order($order);
        $items = array();
        foreach ($order->get_items() as $cart_item_key => $values) {
    ...
    public function paltechwpdevwpc_refund_order($order_id, $amount, $reason, $transaction_id) {
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('PayPal Checkout for WooCommerce Version: %s', 'woo-paypal-checkout'), WOO_PAYPAL_CHECKOUT_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log(sprintf(__('WooCommerce Version: %s', 'woo-paypal-checkout'), WC_VERSION));
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_refund_api . $transaction_id . '/refund');
        WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Refund captured payment => https://developer.paypal.com/docs/api/payments/v2/#captures_refund');
        $order = wc_get_order($order_id);
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        $body_request = array(
            'amount' =>
    ...
    public function paltechwpdevwpc_update_order($order) {
        try {
            $patch_request = array();
            $update_amount_request = array();
            $reference_id = WC()->session->get('paltechwpdevwpc_reference_id');
            $order_id = version_compare(WC_VERSION, '3.0', '<') ? $order->id : $order->get_id();
            $cart = $this->paltechwpdevwpc_get_details_from_order($order_id);
            $old_wc = version_compare(WC_VERSION, '3.0', '<');
            if (( $old_wc && ( $order->shipping_address_1 || $order->shipping_address_2 ) ) || (!$old_wc && $order->has_shipping_address() )) {
                $shipping_first_name = $old_wc ? $order->shipping_first_name : $order->get_shipping_first_name();
                $shipping_last_name = $old_wc ? $order->shipping_last_name : $order->get_shipping_last_name();
            ...
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Test Mode: ' . $this->is_sandbox);
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Endpoint: ' . $this->paypal_order_api . $paypal_order_id);
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Request: ' . print_r($patch_request_json, true));
            WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Action Name : ' . 'Update order => https://developer.paypal.com/docs/api/orders/v2/#orders_patch');
            ...
            if (is_wp_error($response)) {
                $error_message = $response->get_error_message();
                WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout::log('Error Message : ' . wc_print_r($response, true));
                wc_add_notice($error_message, 'error');
                return false;
            } else {
                $api_response = json_decode(wp_remote_retrieve_body($response), true);
    ...
    public function paltechwpdevwpc_get_extra_offset_line_item($amount) {
        $decimals = $this->paltechwpdevwpc_get_number_of_decimal_digits();
        return array(
