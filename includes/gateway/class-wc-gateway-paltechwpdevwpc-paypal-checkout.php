...
if (class_exists('WC_Payment_Gateway')) {
    ...
    class WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout extends WC_Payment_Gateway {

        public static $log_enabled = false;
        public static $log = false;

        public function __construct() {
            $this->id = 'paltechwpdevwpc_paypal_checkout';
            $this->has_fields = false;
            if (paltechwpdevwpc_has_active_session() == true) {
                $this->order_button_text = __('Continue to payment', 'woo-paypal-checkout');
            }
            $this->method_title = __('PayPal Checkout', 'woo-paypal-checkout');
            $this->method_description = __('Add Smart Payment Buttons to your website.', 'woo-paypal-checkout');
            $this->supports = array(
                'products',
                'refunds',
            );
            ...
            add_action('woocommerce_api_wc_gateway_' . $this->id, array($this, 'paltechwpdevwpc_request_handler'));
            add_action('woocommerce_admin_order_totals_after_total', array($this, 'paltechwpdevwpc_display_order_fee'));
        ...
        public function process_admin_options() {
            delete_transient('paltechwpdevwpc_sandbox_access_token');
            delete_transient('paltechwpdevwpc_live_access_token');
        ...
        public function init_form_fields() {
            $this->form_fields = include PALTECHWPDEVWPC_PLUGIN_DIR . '/includes/gateway/settings-paypal-checkout.php';
        }
        ...
        public function process_payment($woo_order_id) {
            if (isset($_POST['from_checkout']) && 'yes' === $_POST['from_checkout']) {
                ...
                $this->request = new WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request($this);
                $this->request->paltechwpdevwpc_create_order_request($woo_order_id);
                exit();
            } else {
                $paltechwpdevwpc_order_id = WC()->session->get('paltechwpdevwpc_order_id');
                if( !empty($paltechwpdevwpc_order_id)) {
                    include_once PALTECHWPDEVWPC_PLUGIN_DIR . '/includes/gateway/class-wc-gateway-paltechwpdevwpc-paypal-checkout-request.php';
                    $this->request = new WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request($this);
                    $order = wc_get_order($woo_order_id);
                    $is_success = $this->request->paltechwpdevwpc_order_capture_request($woo_order_id);
                    if ($is_success) {
                        return array(
                            'result' => 'success',
        ...
        public function process_refund($order_id, $amount = null, $reason = '') {
            ...
            $response = $this->request->paltechwpdevwpc_refund_order($order_id, $amount, $reason, $transaction_id);
        ...
        public function admin_scripts() {
            ...
            wp_enqueue_script('woocommerce_paypal_checkout_admin', PALTECHWPDEVWPC_ASSET_URL . '/admin/js/woo-paypal-checkout-admin' . $suffix . '.js', array(), WC_VERSION, true);
        }

        public function paltechwpdevwpc_request_handler() {
            ...
            if (isset($_GET['pp_action']) && !empty($_GET['pp_action'])) {
                if ($_GET['pp_action'] == 'get_checkout_details' && isset($_GET['orderID']) && !empty($_GET['orderID'])) {
                    include_once PALTECHWPDEVWPC_PLUGIN_DIR . '/includes/gateway/class-wc-gateway-paltechwpdevwpc-paypal-checkout-request.php';
                    $this->request = new WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request($this);
                    $this->request->paltechwpdevwpc_get_checkout_details(wc_clean($_GET['orderID']));
                    wp_safe_redirect(wc_get_checkout_url());
                    exit();
                } else if ($_GET['pp_action'] == 'set_checkout') {
                    if (isset($_POST['from_checkout']) && 'yes' === $_POST['from_checkout']) {
                        WC()->checkout->process_checkout();
                        $this->paltechwpdevwpc_set_customer_data($_POST);
                    } else {
                        include_once PALTECHWPDEVWPC_PLUGIN_DIR . '/includes/gateway/class-wc-gateway-paltechwpdevwpc-paypal-checkout-request.php';
                        $this->request = new WC_Gateway_PALTECHWPDEVWPC_PayPal_Checkout_Request($this);
                    ...
                    if (!empty($paltechwpdevwpc_order_details)) {
                        if (class_exists('Woo_Paypal_Checkout_Public')) {
                            $public_class = new Woo_Paypal_Checkout_Public(WOO_PAYPAL_CHECKOUT_PLUGIN_NAME, WOO_PAYPAL_CHECKOUT_VERSION);
                        } else {
                            include_once PALTECHWPDEVWPC_PLUGIN_DIR . '/public/class-woo-paypal-checkout-public.php';
                            $public_class = new Woo_Paypal_Checkout_Public(WOO_PAYPAL_CHECKOUT_PLUGIN_NAME, WOO_PAYPAL_CHECKOUT_VERSION);
                        }
                        $shipping_details = $public_class->paltechwpdevwpc_get_mapped_shipping_address($paltechwpdevwpc_order_details);
                        $billing_details = $public_class->paltechwpdevwpc_get_mapped_billing_address($paltechwpdevwpc_order_details);
                        $public_class->update_customer_addresses_from_paypal($shipping_details, $billing_details);
                    }
                    $order_id = absint(WC()->session->get('order_awaiting_payment'));
                    $order = wc_get_order($order_id);
                    if ($paltechwpdevwpc_order_details['status'] == 'COMPLETED') {
                        $transaction_id = isset($paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['id']) ? $paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['id'] : '';
                        $seller_protection = isset($paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['seller_protection']['status']) ? $paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['seller_protection']['status'] : '';
                        $currency_code = isset($paltechwpdevwpc_order_details['purchase_units'][0]['payments']['captures'][0]['seller_receivable_breakdown']['paypal_fee']['currency_code']) ? $paltechwpdevwpc_order_details['purchase_units'][0]['payments']['captures'][0]['seller_receivable_breakdown']['paypal_fee']['currency_code'] : '';
                        $value = isset($paltechwpdevwpc_order_details['purchase_units'][0]['payments']['captures'][0]['seller_receivable_breakdown']['paypal_fee']['value']) ? $paltechwpdevwpc_order_details['purchase_units'][0]['payments']['captures'][0]['seller_receivable_breakdown']['paypal_fee']['value'] : '';
                        update_post_meta($order_id, 'paypal_fee', $value);
                        update_post_meta($order_id, 'paypal_fee_currency_code', $currency_code);
                        $payment_status = isset($paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['status']) ? $paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['status'] : '';
                        if ($payment_status == 'COMPLETED') {
                            ...
                            $payment_status_reason = isset($paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['status_details']['reason']) ? $paltechwpdevwpc_order_details['purchase_units']['0']['payments']['captures']['0']['status_details']['reason'] : '';
        ...
        public function paltechwpdevwpc_set_customer_data($data) {
        ...
        public function paltechwpdevwpc_display_order_fee($order_id) {
            $order = wc_get_order($order_id);
